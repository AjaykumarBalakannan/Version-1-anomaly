...
log_file_path = f"logs/V1_USLevel_anomaly_pipeline_{today_str}.log"
...
logging.info("V1 US Level Model pipeline started. Test: This should appear in both log file and console.")
...
if __name__ == "__main__":
    ...
    out_csv = os.getenv('US_OUTPUT_CSV', 'output/anomalies_US_V1.csv')
    batch_size = int(os.getenv('US_BATCH_SIZE', '50000'))

    # Log configuration
    logging.info(f"V1 US Level Model pipeline configuration:")
    ...
    if summary is not None and 'today_anomalies' in summary:
        ...
        if summary['today_anomalies'] is not None:
            today_str = datetime.now().strftime("%Y-%m-%d")
            summary['today_anomalies'].to_csv(f"output/anomalies_today_US_V1_{today_str}.csv", index=False)
    else:
