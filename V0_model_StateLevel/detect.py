...
# This replaces the traditional V0 anomaly detection with ML-based approaches for state-level data
...
class StateAnomalyDetectorV1:
    ...
    def _build_features(df_hist: pd.DataFrame):
        ...
        # Group by state instead of geo_level for state-level analysis
        df = df.groupby(["state", "src"], group_keys=False).apply(
            StateAnomalyDetectorV1._add_roll_feats
        )
        feature_cols = [
    ...
    def _apply_cooldown(df_out: pd.DataFrame, cooldown_days: int) -> pd.DataFrame:
        ...
        def _cool(group):
            ...
            return group
        return df_out.groupby(["state", "src"], group_keys=False).apply(_cool).reset_index(drop=True)

    # ---------- Public API ----------
    def detect_anomalies(self, df_hist: pd.DataFrame, csv_path: str = None):
        """
        Detect anomalies using ensemble ML models for state-level data
        
        Args:
            df_hist: DataFrame with columns ['date', 'state', 'src', 'job_count']
        ...
        Returns:
            ...
            pivot: Pivot table of job counts by state, source and date
        ...
        cols_keep = ["state", "src", "date", "job_count"]
        ...
        thresholds = (
            df_out.groupby(["state", "src"], group_keys=False)
                  .apply(lambda g: pd.Series({"threshold": self._calibrate_threshold_for_group(g, "score_ens")}))
                  .reset_index()
        )
        df_out = df_out.merge(thresholds, on=["state", "src"], how="left")
        df_out["is_anomaly"] = (df_out["score_ens"] >= df_out["threshold"]).astype(int)
        df_out = self._apply_cooldown(df_out, self.cooldown_days)
        
        # Sort by grouping fields (state, src) then by date for logical organization
        df_out = df_out.sort_values(["state", "src", "date"]).reset_index(drop=True)

        pivot = (df_out.pivot_table(index=["state", "src"], columns="date", values="job_count").sort_index())
        ...
        summary = {
            ...
            "segments": int(df_out[["state", "src"]].drop_duplicates().shape[0]),
        ...
        try:
            ...
            if len(today_df):
                summary["today_anomalies"] = today_df.sort_values(["state", "src"])
        except Exception as e:
            self.logger.warning(f"Unable to compute today's anomalies: {e}")

        if csv_path:
            df_out.to_csv(csv_path, index=False)
            self.logger.info(f"v1 state-level anomaly detection results saved to {csv_path}")

        return df_out, pivot, summary


def detect_anomalies_v1(df_hist: pd.DataFrame, csv_path: str = None, **kwargs):
    detector = StateAnomalyDetectorV1(**kwargs)
...
class StateAnomalyDetector:
    ...
    def detect_anomalies(df, csv_path=None, **kwargs):
        ...
        """
        detector = StateAnomalyDetectorV1(
            models=("iforest", "hbos", "copod", "knn", "svm", "ocsvm", "cblof"),
            alert_budget_per_week=2,
            cooldown_days=1,
            quantile_bounds=(0.95, 0.999),
