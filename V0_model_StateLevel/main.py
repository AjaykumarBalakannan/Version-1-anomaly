...
log_file_path = f"logs/V0_StateLevel_anomaly_pipeline_{today_str}.log"
...
logging.info("V0 State Level Model pipeline started. This should appear in both log file and console.")
...
if __name__ == "__main__":
    ...
    out_csv = os.getenv('STATE_OUTPUT_CSV', 'output/anomalies_state.csv')
    batch_size = int(os.getenv('STATE_BATCH_SIZE', '50000'))

    # Log configuration
    logging.info(f"V0 State Level Model pipeline configuration:")
    ...
    if summary is not None and 'today_anomalies' in summary:
        ...
        if summary['today_anomalies'] is not None:
            today_str = datetime.now().strftime("%Y-%m-%d")
            summary['today_anomalies'].to_csv(f"output/anomalies_today_State_{today_str}.csv", index=False)
    else:
