# detect_v1.py
# --- Advanced anomaly detection using PyCaret and PyOD with ensemble modeling for SOC Level ---
...
class SOCAnomalyDetectorV1:
    ...
    def _build_features(df_hist: pd.DataFrame):
        ...
        # Group by nlp_soc_code_rolled instead of state for SOC-level analysis
        df = df.groupby(["nlp_soc_code_rolled", "src"], group_keys=False).apply(
            SOCAnomalyDetectorV1._add_roll_feats
        )
        feature_cols = [
    ...
    def _apply_cooldown(df_out: pd.DataFrame, cooldown_days: int) -> pd.DataFrame:
        ...
        return df_out.groupby(["nlp_soc_code_rolled", "src"], group_keys=False).apply(_cool).reset_index(drop=True)

    # ---------- Public API ----------
    def detect_anomalies(self, df_hist: pd.DataFrame, csv_path: str = None):
        """
        Detect anomalies using ensemble ML models for SOC-level data
        
        Args:
            df_hist: DataFrame with columns ['date', 'nlp_soc_code_rolled', 'src', 'job_count']
        ...
        Returns:
            ...
            pivot: Pivot table of job counts by SOC code, source and date
        ...
        cols_keep = ["nlp_soc_code_rolled", "src", "date", "job_count"]
        ...
        thresholds = (
            df_out.groupby(["nlp_soc_code_rolled", "src"], group_keys=False)
                  .apply(lambda g: pd.Series({"threshold": self._calibrate_threshold_for_group(g, "score_ens")}))
                  .reset_index()
        )
        df_out = df_out.merge(thresholds, on=["nlp_soc_code_rolled", "src"], how="left")
        df_out["is_anomaly"] = (df_out["score_ens"] >= df_out["threshold"]).astype(int)
        df_out = self._apply_cooldown(df_out, self.cooldown_days)
        
        # Sort by grouping fields (nlp_soc_code_rolled, src) then by date for logical organization
        df_out = df_out.sort_values(["nlp_soc_code_rolled", "src", "date"]).reset_index(drop=True)

        pivot = (df_out.pivot_table(index=["nlp_soc_code_rolled", "src"], columns="date", values="job_count").sort_index())
        ...
        summary = {
            ...
            "segments": int(df_out[["nlp_soc_code_rolled", "src"]].drop_duplicates().shape[0]),
        ...
        try:
            ...
            if len(today_df):
                summary["today_anomalies"] = today_df.sort_values(["nlp_soc_code_rolled", "src"])
        except Exception as e:
            self.logger.warning(f"Unable to compute today's anomalies: {e}")

        if csv_path:
            df_out.to_csv(csv_path, index=False)
            self.logger.info(f"v1 SOC-level anomaly detection results saved to {csv_path}")

        return df_out, pivot, summary


def detect_anomalies_v1(df_hist: pd.DataFrame, csv_path: str = None, **kwargs):
    detector = SOCAnomalyDetectorV1(**kwargs)
